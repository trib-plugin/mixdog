 ...
 * Falls back to the legacy per-file JSON when the unified config has no such
 * section (or when the unified config is absent / malformed).
 *
 * @param {string} dataDir  — DATA_DIR passed into build* functions
 * @param {string} section  — top-level key ('memory' | 'search' | …)
 * @param {string} legacyFile — legacy filename (e.g. 'memory-config.json')
 * @returns {object}
 */
function readConfigSection(dataDir, section, legacyFile) {
  try {
    const unified = JSON.parse(fs.readFileSync(path.join(dataDir, 'mixdog-config.json'), 'utf8'));
    if (unified && typeof unified === 'object' && unified[section] != null &&
        !(typeof unified[section] === 'object' && !Array.isArray(unified[section]) && Object.keys(unified[section]).length === 0)) {
      return unified[section];
    }
  } catch {}
  // Fallback: legacy file
  try { return JSON.parse(fs.readFileSync(path.join(dataDir, legacyFile), 'utf8')); } catch { return {}; }
}
...
function buildInjectionContent({ PLUGIN_ROOT, DATA_DIR }) {
  ...
  const memoryConfig = readConfigSection(DATA_DIR, 'memory', 'memory-config.json');
  const searchConfig = readConfigSection(DATA_DIR, 'search', 'search-config.json');
  const parts = [];
