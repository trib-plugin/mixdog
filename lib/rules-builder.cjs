 ...
 *
 * Source files (rules/):
 *   - shared/00-routing.md           — universal routing meta (Lead + bridge)
 *   - shared/02-memory.md            — memory tool detail (recall-agent only, BP3)
 *   - shared/03-search.md            — search tool detail (search-agent only, BP3)
 *   - shared/04-explore.md           — explore tool detail (explorer only, BP3)
 *   - lead/00-tool-lead.md           — Lead-specific tool guidance (delegation + ToolSearch)
 ...
 *   - bridge/02-tool-bridge.md       — bridge tool patterns (BP1, every bridge identical)
...
function buildInjectionContent({ PLUGIN_ROOT, DATA_DIR }) {
  ...
  const routing = readOptional(path.join(SHARED_DIR, '00-routing.md'));
...
function buildBridgeInjectionContent({ PLUGIN_ROOT, DATA_DIR }) {
  ...
  // 1. Universal routing meta.
  const routing = readOptional(path.join(SHARED_DIR, '00-routing.md'));
  if (routing) parts.push(routing);

  // 2. Bridge tool patterns (Edit Ordering, Decision Table, Anti-patterns, soft-warns).
  const toolBridge = readOptional(path.join(BRIDGE_DIR, '02-tool-bridge.md'));
  if (toolBridge) parts.push(toolBridge);

  // 3. Bridge common behavior.
  const common = readOptional(path.join(BRIDGE_DIR, '00-common.md'));
  if (common) parts.push(common);

  // 4. Both family principles — every role sees both, but only the matching
  ...
  // 5. User-defined work-role overrides (DATA_DIR/roles/*.md). Pool-wide.
