...
function _renameWithRetrySync(tmp, target) {
    ...
    // Unlink-before-write and copy fallbacks risk truncating the live file.
    // Throw so callers know the persist failed rather than silently corrupting.
    if (lastErr) {
        try {
            process.stderr.write(`[session-store] rename fallback used for ${target}: ${lastErr.code || lastErr.message}\n`);
        } catch { /* ignore logging failure */ }
    }
    throw lastErr || new Error(`rename failed for ${target}`);
}
