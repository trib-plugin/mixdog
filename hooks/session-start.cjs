...
// cmd.exe is done parsing it. Earlier attempts passed two separately-quoted
// paths (bash.exe + script) — cmd's outer-quote-stripping rule (CommandLineToArgvW
// combined with the `cmd /c "..."` wrapper Claude Code uses) mangled the argv
// and silently failed to launch, so line 2 never rendered.
//
// Fix: emit a .bat wrapper alongside statusline.sh and point `statusLine.command`
// at the .bat path only. cmd handles .bat execution without quote gymnastics;
// inside the .bat we invoke bash.exe with "%~dp0statusline.sh" so the script
// path never needs to be embedded at injection time. The .bat is rewritten
// on every session start to pick up the correctly detected Git Bash executable
// (handles PROGRA~1, PROGRA~2, and 8dot3name-disabled machines).
//
...
// Rewrites PLUGIN_ROOT/bin/statusline.bat to invoke the detected bash.exe.
// Returns the absolute .bat path (Windows-native backslashes) for use as the
// single-token statusLine command, or null if no Git Bash could be detected.
...
function resolveStatusLineCommand(pluginRoot, scriptPath) {
  ...
  // Fallback: no Git Bash detected. Keep the bare `bash` form — behavior matches
  // a machine without Git Bash installed (statusline will be empty but nothing breaks).
