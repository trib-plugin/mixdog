...
//
// Claude Code runs statusLine commands via a shell on every platform —
// Git Bash on Windows, /bin/sh elsewhere. Both honor shebangs and the
// executable bit, so pointing `command` at statusline.sh directly (forward-slash
// absolute path, no wrapper) is enough. Earlier 0.1.30 emitted a .bat wrapper
// thinking cmd.exe was the executor; it isn't, and Git Bash cannot execute
// .bat files, which is why the statusline never rendered.
