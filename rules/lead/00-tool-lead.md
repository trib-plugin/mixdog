...
- When reading or editing mixdog plugin source itself, pass `cwd: '~/.claude/plugins/marketplaces/trib-plugin'` explicitly. Path access is governed by Claude Code's `settings.json` permissions. mcp tools accept any path the user permission allows; mcp server only hard-blocks dangerous patterns (UNC, parent escape, system paths).
