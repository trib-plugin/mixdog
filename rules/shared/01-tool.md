...
- `find_symbol` default already returns file-grouped reference candidates with `matches=N` counts; calling `mode:"references"` in addition is duplicate. Use references mode only when line-level details for every reference are required.
- `explore` synthesis cost is high (internal agent fan-out). When the target coordinate (file/symbol/path) is guessable, prefer `grep` / `find_symbol` for one cheap turn. Reserve `explore` for genuinely unknown locations or cross-cutting questions.
- `find_symbol` `mode:"callers"`/`"references"`/`"dependents"` returning empty for a recently-created or recently-edited file is **inconclusive**, not authoritative — likely cause is **cwd mismatch**. The graph cache is keyed by cwd; pass the explicit `cwd` parameter exposed on `find_symbol` (with mode parameter) (tools.json:1367) to target the correct project root. Invalidation hooks are wired but only fire when the same cwd is used; if results remain empty after setting `cwd`, confirm with a targeted `grep`.
...
- Same file, multiple chunks (different `offset`/`limit` on the same path) → MUST use `read({path:[{path,offset,limit},{path,offset,limit}]})` (object-array form), not N parallel `read` tool_use blocks. Splitting same-file chunks across separate tool_use slots inflates iter accounting and is a violation.
- 2+ files in one turn with shared opts → `read({path:[a,b,c]})`. Per-file different `offset`/`limit`/`mode` → `read({path:[{path,offset,limit,mode?,n?}, …]})` (object-array form on the same `path` parameter). Same parallelism rule applies to `edit` (`edits` array), `write` (`writes` array), `grep`/`glob` (pattern array).
- 2+ hunks across multiple files (cross-file refactor, coordinated rename, import + definition move) → prefer `apply_patch` atomic over `edit({edits:[…]})`. Atomic apply gives all-or-nothing semantics + context validation, which avoids the snapshot-stale retry loop that hits `edit` when files were modified between read and write (common after `dev-sync` or other writers touched the file). **Same-file 2+ hunks via `edit edits[]` are safe**: the handler reads the file once, applies all hunks in memory, and writes once atomically — no inter-hunk snapshot-stale window.
