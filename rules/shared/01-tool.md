...
- `find_symbol` `mode:"callers"`/`"references"`/`"dependents"` returning empty for a recently-created or recently-edited file is **inconclusive**, not authoritative — the code graph cache may be stale. Confirm with one targeted `grep` on the symbol name before reporting `not found`. Note: code-graph indexing is **not automatic** for new files; the cache is rebuilt only via `_internals.persistCodeGraphDiskCacheNow` / `resetCodeGraphCachesForTesting` (code-graph.mjs:1757-1761). Newly created modules can stay graph-stale indefinitely — for new code, default to `grep` and skip the `find_symbol` retry entirely.
