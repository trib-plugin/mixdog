...
- `find_symbol` `mode:"callers"`/`"references"`/`"dependents"` returning empty for a recently-created or recently-edited file is **inconclusive**, not authoritative — likely cause is **cwd mismatch**. The graph cache is keyed by cwd (`_codeGraphCache` in `code-graph.mjs:40`); when the caller's cwd does not match the file's project root, the lookup runs against an empty/foreign graph and returns no results regardless of how recent the actual write was. Invalidation hooks are wired (`markCodeGraphDirtyPaths` from `bash-session.mjs:568`, `patch.mjs:657`, `builtin.mjs:2849/3134/3542/3600/3830`) and `_consumeCodeGraphDirtyPaths` is called inside `_buildCodeGraph` (code-graph.mjs:1347-1353) — the dirty-path infra works *if* the same cwd is used. Lead's default cwd (e.g. `C:\Project`) typically differs from the plugin marketplace root, so structural queries against plugin source from Lead context are graph-blind. Workaround: confirm with a targeted `grep` and treat the structural result as a hint, not ground truth, until find_symbol/code_graph expose an explicit `cwd` parameter.
